...
-- Postgres requires index expressions to be IMMUTABLE. date_trunc and
-- to_char are both STABLE (they pass through timestamp). EXTRACT from a
-- plain date IS IMMUTABLE, so we bucket by (year, month) as two columns.
